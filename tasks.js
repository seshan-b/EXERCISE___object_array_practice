...
function releasesByArtist (artistArrObjFile, releasesArrObjFile, artistName) {
  ...
  const findArtistName = artistArrObjFile.find(
    (element) => element.name === artistName
  )
...
function getGenreByArtist (listenerArrObjFile, artistArrObjFile, listenerName) {
  ...
  const findAListenerName = listenerArrObjFile.find(
    (element) => element.name === listenerName
  )
  ...
  for (let i = 0; i < getGenre.length; i++) {
    const getGenreData = artistArrObjFile.filter(
...
function getReleaseByArtistGenre (
  listenerArrObjFile,
  releaseArrObjFile,
  listenerName
) {
